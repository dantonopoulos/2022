$PBExportHeader$n_podiasrf_connectservice.sru
$PBExportComments$Generated Database Connection Service
forward
global type n_podiasrf_connectservice from nonvisualobject
end type
end forward

global type n_podiasrf_connectservice from nonvisualobject
end type
global n_podiasrf_connectservice n_podiasrf_connectservice
...
Choose Case is_connectfrom
	...
	Case IS_USE_INIFILE						/*  Populate Database Connection from INI file  */
		string ls_inifile = "podiasrf.ini"
		...
		as_logpass		= ProfileString ( ls_inifile, "Database", "LogPassword",	"futton")
		as_server		= ProfileString ( ls_inifile, "Database", "Servername", "hp370g4")
		as_dbparm		= ProfileString ( ls_inifile, "Database", "DBParm", "Release='15'")
		as_lock			= ProfileString ( ls_inifile, "Database", "Lock", "")
		as_autocommit	= ProfileString ( ls_inifile, "Database", "AutoCommit",	"true")
		gs_log_level     = ProfileString ( ls_inifile, "OtherInfo", "LogLevel",	"1")
		gi_log_level 		= long(gs_log_level)
		gs_load_what   = ProfileString ( ls_inifile, "OtherInfo", "LoadWhat",	"1")
		gl_load_what 	= long(gs_load_what)
	...
	Case IS_USE_REGISTRY						/*  Populate Database Connection from Registry  */
		...
		If RegistryGet ( ls_registrykey, "Database", 	RegString!, as_database ) <> 1 Then
			RegistrySet ( ls_registrykey, "Database", 	RegString!, "aardvark2002" )
		...
		If RegistryGet ( ls_registrykey, "LogPassword", RegString!, as_logpass ) <> 1 Then
			RegistrySet ( ls_registrykey, "LogPassword", RegString!, "dev2021" )
	...
	Case IS_USE_SCRIPT							/*  Populate Database Connection from Script  */
		...
		as_database		= "aardvark2002"
		as_userid		= ""
		as_dbpass		= ""
		as_logid			= "sa"
		as_logpass		= "dev2021"
		as_server		= "syb15"
		as_dbparm		= "Release='15'"
		as_lock			= ""
		as_autocommit	= "false"
...
If SQLCA.SQLCode <> 0 Then
	...
	fx_log_file('Cannot Connect to Database' + SQLCA.SQLErrText,0)
...
on n_podiasrf_connectservice.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_podiasrf_connectservice.destroy
